...
cart.forEach((cartItem) => {
  ...
  // creates HTML for products at Checkout page 
  cartSummaryHTML +=
    `
...
// updates the cart Quantity in top Header of Checkout page
...
document.querySelectorAll('.save-quantity-link').forEach((link) => {
  ...
  // save the quantity from input of update....
  link.addEventListener('click', () => {
    const productId = link.dataset.productId;
    ...
    // auto deletes the container if Qunatity is "0"
    if (newQuantity == 0) {
      const productId = link.dataset.productId;
      removeFromCart(productId);
      const container = document.querySelector(`.js-cart-item-container-${productId}`);
    ...
    // removes the input and save links .... when click save
    const container = document.querySelector(`.js-cart-item-container-${productId}`);
